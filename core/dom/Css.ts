...
export namespace Css
{
    ...
    export class     Rule
    {
        ...
        )
        {
            ...
            if (type === '@keyframes' || type === '@page')
                ...
                for (const [k, v] of Object.entries(contents))
                    this.#properties[Text.toCamel(k)] = typeof v === 'string' ? v.trim() : String(v).trim();
         ...
         *  @description Read one property's value. The name is camelCased first (Text.toCamel), so
        ...
        {
            return this.#properties[Text.toCamel(name)];
        }
        ...
        replace(props: Record<string, string> | string): this
        {
            ...
            for (const [k, v] of Object.entries(parsed))
            {
                ...
                this.#properties[Text.toCamel(k)] = p;
                this.#invalidate();
            }
         ...
         *  @description Test whether a property is present. The name is camelCased first (Text.toCamel), so
         *               `'font-size'` and `'fontSize'` query the same property.
         *  @author      Riccardo Angeli
         *  @copyright   Riccardo Angeli 2012-2026 All Rights Reserved
         *  @license     MIT / Commercial (dual license) */
        has(name: string): boolean { return Text.toCamel(name) in this.#properties; }
        ...
        {
            Services.Events?.Fire(this.#selector, {
                Type       : type,
        ...
        {
            Services.Events?.On(this.#selector, types, handler, options);
            return this;
        }
        ...
        {
            Services.Events?.Off(this.#selector, types, handler);
            return this;
        }
        ...
        {
            return Object.entries(props).map(([k, v]) => `${Text.toKebab(k)}: ${v}`).join('; ');
        }
        ...
        static GetSelector(def: RuleInterface): string
        {
            ...
            const cond = (obj: Record<string, unknown>): string =>
            {
                ...
                for (const [k, v] of Object.entries(obj))
                {
                    ...
                    else                   parts.push({ c: null,  t: `(${Text.toKebab(k).toLowerCase()}: ${v})` });
                }
                let out = '';
                parts.forEach((p, i) =>
                {
        ...
        static GetContents(def: RuleInterface): Record<string, unknown>
        {
            ...
            body.split(';').forEach(decl => {
                ...
                if (colon < 0) return;
                const key = Text.toCamel(decl.slice(0, colon).trim());
        ...
        ): string
        {
            ...
            const norm = (raw: unknown): Record<string, string> =>
            {
                ...
                {
                    out[Text.toCamel(key)] = String(value).trim();
                }
            ...
            if (type === '@page')
            {
                ...
                for (const [key, value] of Object.entries(contents))
                {
                    ...
                    {
                        main[Text.toCamel(key)] = String(value).trim();

                        continue;
                    }

                    const box          = Text.toKebab(key).toLowerCase();
        ...
        static GetObject(cssText: string): Record<string, unknown>
        {
            ...
            const collect = (style: { length: number; [i: number]: string; getPropertyValue(p: string): string }): Record<string, string> =>
            {
                ...
                {
                    const p = style[i] ?? '';
                    if (p) d[Text.toCamel(p)] = style.getPropertyValue(p).trim();
                }
                return d;
            };
    ...
    export class     Stylesheet
    {
            ...
            try
            {
                for (const rule of Array.from(this.#sheet.cssRules))
                {
                    if (rule instanceof CSSStyleRule)
                    {
                        ...
                        {
                            const prop = decl[i] ?? '';
                            if (prop) out[Text.toCamel(prop)] = decl.getPropertyValue(prop).trim();
                        }
                    }
                }
        ...
        {
            Services.Events?.On(document, types, handler, options);
            return this;
        }
        ...
        off(types: string, handler: EventListener): this
        {
            Services.Events?.Off(document, types, handler);
            return this;
        }
