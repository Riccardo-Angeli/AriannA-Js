...
export namespace Namespaces
{
     ...
     *  @copyright   Riccardo Angeli 2012-2026 All Rights Reserved
     *  @license     MIT / Commercial (dual license) */
    export type Base           = SchemaTypes.DOM.Base;
    /** @name        Constructor
     *  @public
     *  @type        {type alias}
     ...
     *  @copyright   Riccardo Angeli 2012-2026 All Rights Reserved
     *  @license     MIT / Commercial (dual license) */
    export type Constructor    = SchemaTypes.DOM.Constructor;
    /** @name        IDL
     *  @public
     *  @type        {type alias}
     ...
     *  @copyright   Riccardo Angeli 2012-2026 All Rights Reserved
     *  @license     MIT / Commercial (dual license) */
    export type IDL            = SchemaTypes.DOM.IDL;
    /** @name        TypeOptions
     *  @public
     *  @type        {type alias}
     ...
     *  @copyright   Riccardo Angeli 2012-2026 All Rights Reserved
     *  @license     MIT / Commercial (dual license) */
    export type TypeOptions    = SchemaInterfaces.Core.TypeOptions;
    ...
    {
        /** Complete prototype chain, most-derived first. */
        static GetPrototypeChain(value: object | Function | null | undefined, mode?: 's' | 'STRINGS'): string[];
        static GetPrototypeChain(value: object | Function | null | undefined, mode: 'f' | 'FUNCTIONS'): Function[];
        static GetPrototypeChain(value: object | Function | null | undefined, mode: 'y' | 'SYMBOLS'): symbol[];
        static GetPrototypeChain
        (
            value: object | Function | null | undefined,
            mode: 's' | 'STRINGS' | 'f' | 'FUNCTIONS' | 'y' | 'SYMBOLS' = 's'
        ): string[] | Function[] | symbol[]
        {
            const m = mode.toLowerCase();
            const functionsMode = m === 'f' || m === 'functions';
            const symbolsMode = m === 'y' || m === 'symbols';
            const strings: string[] = [];
            const functions: Function[] = [];
            const symbols: symbol[] = [];
            if(value == null) return functionsMode ? functions : symbolsMode ? symbols : strings;
            const prototype = (value as { prototype?: object }).prototype;
            let proto: object | null = typeof value === 'function' ? (prototype ?? null) : Object.getPrototypeOf(value);
            while(proto !== null)
            {
                const constructor = (proto as { constructor?: Function }).constructor;
                if(constructor)
                {
                    if(functionsMode) functions.push(constructor);
                    else if(symbolsMode) { if(constructor.name) symbols.push(Symbol.for(constructor.name)); }
                    else strings.push(constructor.name || '[Anonymous]');
                }
                proto = Object.getPrototypeOf(proto);
            }
            return functionsMode ? functions : symbolsMode ? symbols : strings;
        }

        /** Native/class/interface predicate owned by the namespace/type system. */
        static Is
        (
            value: unknown,
            ...matches: string[] | Array<SchemaTypes.DOM.Native> | readonly SchemaTypes.DOM.Base[]
        ): boolean
        {
            if(matches.length === 0) return false;
            const orMode = matches.length === 1 && Array.isArray(matches[0]);
            const types = (orMode ? matches[0] : matches) as readonly SchemaTypes.DOM.Native[];
            const chain = Namespace.GetPrototypeChain(value as object | Function | null | undefined);
            ...
            for(const type of types)
            {
                ...
                if(typeof type === 'string')
                {
                    ...
                    if(typeof value === 'function')
                    {
                        ...
                        if(upper === 'CLASS') match = /^\s*class[\s{]/.test(source);
                        else if(upper === 'IDL')
                        {
                            const element = globalThis.Element;
                            match = /\[native code\]/.test(source) && !!element && (value === element || chain.includes(element.name));
                        }
                        else if(upper === 'IDL-PATCHED')
                        {
                            ...
                            if(name)
                            {
                                for(const namespace of Object.values(Namespace.Namespaces))
                                {
                                    const descriptor = namespace.Standard.get(name);
                                    if(descriptor?.Patched) { match = true; break; }
                                }
                            }
                        }
                    }
                    else match = typeof value === type;
                }
                else if(typeof type === 'function')
                {
                    match = value === type || (!!type.name && chain.includes(type.name));
                }

                if(orMode && match) return true;
                if(!orMode && !match) return false;
            }
            return !orMode;
        }
                    ...
                    const d : SchemaType =
                        {
                            ...
                            Stylesheet  : '',
                            Native      : true,
                            Chain       : constructor ? Namespace.GetPrototypeChain(constructor) : [],
                            State       : 'Success',
                            Slot        : 'External',
                            Properties  : [],
                            Methods     : [],
         ...
         *  @license     MIT / Commercial (dual license)
         */
        GetDeclaration(value: unknown): SchemaTypes.DOM.Declaration
        {
            if (Namespace.Is(value as object, 'idl'))   return 'IDL';
            if (Namespace.Is(value as object, 'idl-patched'))   return 'IDL';
            if (Namespace.Is(value as object, 'class')) return 'CLASS';
            return 'FUNCTION';
        }
            ...
            if(t && c && b)
            {
                ...
                if(!nv && !nr)
                {
                    ...
                    if(!et && !ec)
                    {
                         ...
                         *
                         * `Namespace.Is(b,'idl-patched')` is true only for a registered Standard
                        ...
                        const ci = Namespace.Is(c, 'idl-patched');
        ...
        private Promote(d: SchemaType): SchemaType | false
        {
            ...
            d.Prototype = cp;
            d.Chain     = Namespace.GetPrototypeChain(c);
            d.State     = 'Success';
            d.Defined   = true;
         ...
         *               decided by `Namespace.Is(x, 'idl')` — the same `[native code]` +
        ...
        GetNativeInterface(Constructor: Base): IDL | false
        {
            ...
            { if (Namespace.Is(c, 'idl-patched')) return c as IDL; }
            return false;
        }
    }

    /** Minimal service adapter consumed by Natives/Core without moving registry ownership back into Core. */
    const NamespaceService: SchemaInterfaces.Services.NamespacesService =
    {
        ...
        Resolve(value: unknown): SchemaType | false
        {
            return Namespace.Resolve(value as string | Base | Node | object);
        },

        Create(name: string, descriptor: Partial<Descriptor>): Namespace
        {
            return Namespace.Namespaces[name] ?? new Namespace(name, descriptor);
        },

        Upgrade(node: Element, descriptor: SchemaType): Element | false
        {
            const owner = Namespace.Namespaces[descriptor.Namespace];
            return owner ? owner.Upgrade(node, descriptor) : false;
        },

        FindStandard(name: string): SchemaType | undefined
        {
            ...
            {
                const descriptor = namespace.Standard.get(name)
                    ?? namespace.Standard.Tags.get(key)
                    ?? Array.from(namespace.Standard.values()).find
                    (
                        candidate => (candidate as SchemaType).Name.toLowerCase() === key
                    );
    ...
    new Services.Service<SchemaInterfaces.Services.NamespacesService>('namespaces', NamespaceService);
}
