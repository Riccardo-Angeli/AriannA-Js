/**
 * @module    core/components/Virtual
...
export namespace Virtuals
{
     ...
     *  @copyright   Riccardo Angeli 2012-2026 All Rights Reserved
     *  @license     MIT / Commercial (dual license) */
    export type NodeType          = Types.Virtuals.NodeType;
    /** @name        Target
     *  @public
     *  @type        {type alias}
     ...
     *  @copyright   Riccardo Angeli 2012-2026 All Rights Reserved
     *  @license     MIT / Commercial (dual license) */
    export type Target            = Types.Virtuals.Target;
    /** @name        Child
     *  @public
     *  @type        {type alias}
     ...
     *  @copyright   Riccardo Angeli 2012-2026 All Rights Reserved
     *  @license     MIT / Commercial (dual license) */
    export type Child             = Types.Virtuals.Child;
    /** @name        Attributes
     *  @public
     *  @type        {type alias}
     ...
     *  @copyright   Riccardo Angeli 2012-2026 All Rights Reserved
     *  @license     MIT / Commercial (dual license) */
    export type Definition        = Interfaces.Virtuals.Definition;
     ...
     *  @copyright   Riccardo Angeli 2012-2026 All Rights Reserved
     *  @license     MIT / Commercial (dual license) */
    export type ServiceContract   = Interfaces.Virtuals.Service;
    /** @name        Signal
     *  @public
     *  @type        {type alias}
     ...
     *  @copyright   Riccardo Angeli 2012-2026 All Rights Reserved
     *  @license     MIT / Commercial (dual license) */
    export type SchemaType        = Interfaces.Namespaces.Type;
...
// ── Deferred-mount path (Jsx.ts Snabbdom vnode / React element) ──────
            ...
            if(def instanceof Templates.Template)
            {
                ...
                if(!template)
                {
                    throw new TypeError
                    (
                        '[arianna] Virtual cannot clone a compiler-only Template without a materialized Template node.'
                    );
                }
                ...
                {
                    const host =
                        Reals.Real.Create('div');

                    Reals.Real.Append(host, fragment);
        ...
        render(): Element
        {
            ...
            this.#dom = (d && (d as { Custom?: boolean }).Custom && typeof Namespaces.Namespace.Create === 'function')
                ? ((Namespaces.Namespace.Create(this.#tag) as Element) ?? Reals.Real.Create(this.#tag))
                : Reals.Real.Create(this.#tag);

            // Flush attribute buffer
            for (const [k, v] of Object.entries(this.#attrs))
            {
                if (v !== null) Reals.Real.Attribute(this.#dom, k, String(v));
            }

            // Flush text buffer
            if (this.#text) Reals.Real.Content(this.#dom, this.#text);

            // Recursively render children and append
            for (const child of this.#children)
            {
                Reals.Real.Append(this.#dom, child.render());
            }
            ...
            for (const sink of this.#sinks)
            {
                switch (sink.type)
                {
                    case 'text':
                    {
                        ...
                        Reals.Real.Append(this.#dom, node);
                        this.#effects.push(Virtual.effect(() => {
                            Reals.Real.Text(node, (sink.getter as (() => string))());
                        }));
                        break;
                    }
                    case 'textMono':
                    {
                        const node = sink.node ?? Reals.Real.CreateText(sink.mono!.Peek());
                        if (!sink.node) Reals.Real.Append(this.#dom, node);
                    ...
                    case 'attr':
                    {
                        ...
                        this.#effects.push(Virtual.effect(() => {
                            ...
                            Reals.Real.Attribute(el, sink.name!, v);
                        }));
                        break;
                    }
                    case 'cls':
                    {
                        const el = this.#dom;
                        this.#effects.push(Virtual.effect(() => {
                            Reals.Real.Class(el, sink.name!, (sink.getter as (() => boolean))());
                        }));
                        break;
                    }
                    case 'prop':
                    {
                        ...
                        (
                            ...
                            (
                                () =>
                                {
                                    Reals.Real.Property(element, sink.name!, sink.getter());
                                }
                            )
                        );
                    ...
                    case 'style':
                    {
                        ...
                        this.#effects.push(Virtual.effect(() => {
                            Reals.Real.Style(el, p, (sink.getter as (() => string))());
                        }));
                        break;
                    }
                    case 'bind':
                    {
                        ...
                        if(sink.setter)
                        {
                            ...
                            (
                                ...
                                event =>
                                {
                                    sink.setter!((event.target as HTMLInputElement).value);
                                }
                            );
                        }
        ...
        ): this
        {
            ...
            if (p) Reals.Real.Append(p, this.render());
            this.#mounted = true;
            return this;
        }
        ...
        unmount(): this
        {
            ...
            {
                if (this.#deferredNode) Reals.Real.Remove(this.#deferredNode);
            }

            this.#deferredNode = null;
            if (this.#dom) Reals.Real.Remove(this.#dom);
            this.#mounted = false;
        ...
        add(...args: ((Virtual | string | number | boolean | null | undefined) | number)[]): this
        {
            ...
            if (this.#dom)
            {
                ...
                for (const n of vnodes) Reals.Real.Append(frag, n.render());
                Reals.Real.Before(this.#dom, frag, ref);
            }
            return this;
        }
        ...
        remove(...targets: (string | number | Virtual)[]): this
        {
            for (const t of targets)
            {
                ...
                else if (t instanceof Virtual)
                {
                    ...
                    if (t.#dom) Reals.Real.Remove(t.#dom);
                }
            }
            return this;
        }
        ...
        /**
         * Read one attribute from the buffered Virtual definition or the rendered Element.
         * Nested dotted-path access no longer belongs to Virtual in Architecture 2.0.
         */
        ...
        /**
         * Write one direct DOM property/attribute. Before render the value is buffered as an attribute;
         * after render known DOM properties are written through Real and everything else is an attribute.
         * Dotted-path/sub-accessor mutation was removed from Architecture 2.0.
         */
        set
        (
            name  : string,
        ...
        ): this
        {
            if(this.#dom)
            {
                ...
                {
                    Reals.Real.Property(this.#dom, name, value);
                }
                else
                {
        ...
        {
            if (this.#dom) Reals.Real.Style(this.#dom as HTMLElement, prop, val);
            return this;
        }
         ...
         *  @public
         *  @memberof    Virtual
         *  @param       {'open' | 'closed'} [mode='open'] Shadow root mode.
         ...
         *  @copyright   Riccardo Angeli 2012-2026 All Rights Reserved
         *  @license     MIT / Commercial (dual license) */
        shadow(mode: 'open' | 'closed' = 'open', options: Record<string, unknown> = {}): this
        {
            if(this.#dom)
            {
                Services.Call
        ...
        textMono(s: SignalMono<string>, node?: Text): this
        {
            if (this.#dom)
            {
                const n = node ?? Reals.Real.CreateText(s.Peek());
                if (!node) Reals.Real.Append(this.#dom, n);
                s.BindText(n);
            }
            else
            {
        ...
        attr(name: string, getter: (() => string | null) | string | null): this
        {
            ...
            if (this.#dom)
            {
                ...
                this.#effects.push(Virtual.effect(() => {
                    const v = g();
                    Reals.Real.Attribute(el, name, v);
                }));
            }
            else
            {
            ...
            if (typeof propOrThing === 'string')
            {
                ...
                else if (propOrThing.indexOf(':') !== -1)
                {
                    ...
                    if (this.#dom)
                    {
                        ...
                        Reals.Real.Attribute(el, 'style', cur + ';' + propOrThing);
                    }
                    else
                    {
        ...
        set Sheet(next: Stylesheet | null)
        {
            ...
            {
                Reals.Real.Remove(this.#styleNode);
            }
            this.#styleNode = null;
            this.#sheetSync = null;
            this.#sheet     = next;
            ...
            const apply = () =>
            {
                ...
                if (!this.#styleNode)
                {
                    this.#styleNode = Reals.Real.Create('style') as HTMLStyleElement;
                    Reals.Real.Attribute(this.#styleNode, 'data-arianna-sheet', el.tagName.toLowerCase());
                    Reals.Real.Attribute(this.#styleNode, 'data-arianna-instance', this.#instanceId);
                    if (useShadow)
                    {
                        Reals.Real.Append((el as Element & { shadowRoot: ShadowRoot }).shadowRoot, this.#styleNode);
                    }
                    else
                    {
                        Reals.Real.Append(document.head ?? document.documentElement, this.#styleNode);
                    }
                }
                Reals.Real.Content(this.#styleNode, css);
            };
