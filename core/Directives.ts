...
export namespace Directives
{
            ...
            return function <T extends typeof HTMLElement>(Base: T): T
            {
                ...
                const _render = function (this: HTMLElement) {
                    ...
                    if (meta.shadow !== false) {
                        ...
                        if (!root) {
                            try { root = this.attachShadow({ mode }); }
                            catch { root = null; }
                        }
                        if (root) {
                            roots.set(this, root);
                            if (styleText)    { const s = document.createElement('style'); s.textContent = styleText; root.appendChild(s); }
                            if (templateText) { const t = document.createElement('template'); t.innerHTML = templateText; root.appendChild(t.content.cloneNode(true)); }
                            return;
                        }
                    ...
                    if (styleText) {
                        ...
                        s.textContent = styleText.replace(/:host/g, meta.tag);
                        document.head.appendChild(s);
                    }
                    if (templateText && !this.children.length) this.innerHTML = templateText;
                };
    ...
    (
        'directives',
        {
            ...
            (
                name  : string,
                apply : (element: Element, value: unknown, options?: SchemaInterfaces.Directive.Options) => () => void
            ): Directive
            {
                const cleanups =
            ...
            (
                root?    : ParentNode,
                options? : SchemaInterfaces.Directive.Options
            ): () => void
            {
                const target =
