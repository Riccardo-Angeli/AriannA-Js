...
/*
 * Template bindings historically consume effect() as a function returning a
 * disposer. Reactivity.Effect is now nominal, so this adapter preserves the
 * existing binding lifecycle without changing every binding implementation.
 */
const effect =
    (
        run: () => void
...
/** @namespace Templates @description Internal binding/compile types, merged with the class. */
...
// per-iteration scope (signals + iter variable).
    ...
    export type CompiledExpr = (this: object, signals: Record<string, unknown>) => unknown;

    /** @name TemplateDescriptor @public @interface
     *  @description Template subsystem descriptor — the static structure a type instantiates
     *  per element. Canonical home of the shape formerly declared inline in `Core`
     *  (`Descriptors.Template`); Core now takes it via `import type`.
    ...
    {
        /** @name Ref @public @type {string=} @description Template id resolved at runtime against the template registry. */
        Ref?  : string;
        /** @name Html @public @type {string=} @description Inline template markup instantiated per element. */
        Html? : string;
        /** @name Mode @public @type {('clone'|'compile')=} @description Instantiation strategy: `clone` a `<template>`, or `compile` from IR. */
        Mode? : 'clone' | 'compile';
    }

}

// Top-level re-exports (Virtual imports { Template }; html/css/TemplateInstance kept).
export import Template         = Templates.Template;
export import TemplateInstance = Templates.TemplateInstance;
export const html = Templates.html;
export const css  = Templates.css;
...
/** @name templateService @private @description Registers the 'template' service: html`` / css`` tagged builders. */
const Service =  new Core.Services.Service
(
    'template',
    {
        /** Tagged-template → compiled Template. */
        html,
        /** Tagged-template → scoped CSS string. */
        css,
    }
);

export default Templates.Template;
