...
export class Sheet
{
     ...
     *     bundle, since Sheet.ts imports it at the top), the input is
    ...
    #flushRules(): void
    {
        ...
        // IMPORTANT: use the LIVE cssRules.length as insertion index, not
        // the forEach `i`. Browsers may reject individual rules (e.g.
        // ::-moz-selection on Chrome) — when that happens, cssRules.length
        // does NOT grow, so using `i` would make every subsequent insertRule
        // call fail with IndexSizeError. Always inserting at the current
        // length keeps the index valid even when some rules are dropped.
