...
export class NodeEditor extends HTMLElement {
    ...
    #typeCheck: TypeCheckFn = (srcType, dstType) => (srcType === dstType || srcType === 'any' || dstType === 'any') ? 'connected-ok' : 'connected-error';
...
/* ──────────────────────────────────────────────────────────────────────────
 * NodeEditor namespace — public component contracts and module helpers.
 * ────────────────────────────────────────────────────────────────────────── */
export namespace NodeEditor {
    export namespace Types {
        export type WireStatusType = WireStatus;
        export type RunStateType = RunState;
        export type TypeCheckFnType = TypeCheckFn;
    }
    export namespace Interfaces {
        export interface PortSpecContract extends PortSpec {
        }
        export interface ParamSpecContract extends ParamSpec {
        }
        export interface NodeSchemaContract extends NodeSchema {
        }
        export interface NodeInstanceContract extends NodeInstance {
        }
        export interface WireInstanceContract extends WireInstance {
        }
        export interface Options extends NodeEditorOptions {
        }
    }
}
