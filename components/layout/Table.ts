...
export class Table extends HTMLElement {
    ...
    onConnected(_opts: TableOptions = {}) {
    ...
    exportCSV(filename = 'table-export.csv'): string {
        const cols = this.columns$.Get().filter(c => this.visibility$.Get()[c.key] !== false && c.visible !== false);
        ...
        const body = sourceRows.map(row => cols.map(c => {
    ...
    #processClientSide(rows: Row[], _alreadyPaged: boolean): Row[] {
        ...
        if (q) {
            ...
            filtered = rows.filter(row => cols.some(col => {
...
/* ──────────────────────────────────────────────────────────────────────────
 * LRU namespace — public component contracts and module helpers.
 * ────────────────────────────────────────────────────────────────────────── */
export namespace LRU {
}
/* ──────────────────────────────────────────────────────────────────────────
 * Table namespace — public component contracts and module helpers.
 * ────────────────────────────────────────────────────────────────────────── */
export namespace Table {
    export namespace Types {
        export type RowType = Row;
        export type SortDirType = SortDir;
        export type SelectModeType = SelectMode;
        export type FetcherType = Fetcher;
    }
    export namespace Interfaces {
        export interface Column<R = Row> extends TableColumn<R> {
        }
        export interface SortStateContract extends SortState {
        }
        export interface FetchParamsContract extends FetchParams {
        }
        export interface FetchResultContract extends FetchResult {
        }
        export interface Options extends TableOptions {
        }
        export interface DisplayRowContract extends DisplayRow {
        }
        export interface HeaderCellContract extends HeaderCell {
        }
        export interface PageBtnContract extends PageBtn {
        }
        export interface ColToggleEntryContract extends ColToggleEntry {
        }
    }
    export const CsvCell = csvCell;
    export const GetWorkerUrl = getWorkerUrl;
}
