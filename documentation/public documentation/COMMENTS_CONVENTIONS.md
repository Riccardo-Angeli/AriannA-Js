...
### 1.1 Member JSDoc — mandatory tag set

All documentation is written in **English**, with a full JSDoc block on **every member** — no member is left bare. The canonical tag set (see `Component.js` for the reference) is applied uniformly:

- **File header** — `@module`, `@author`, `@copyright`, `@license`, then a one-line description and a short context paragraph.
- **Class / interface header** — `@author`, `@copyright`, `@license`, `@class` (classes) or `@interface` (interfaces), `@memberof` when nested, and a `@classdesc` / description.
- **Every member** (field, accessor, method) — `@name`, an access tag (`@public` / `@private` / `@readonly`), `@type`, `@description`, `@author`, `@copyright`, `@license`. Add `@default` when a default exists, and `@param` / `@returns` for function-typed members.

```ts
/** @name        State
 *  @public
 *  @type        {'Fail' | 'Warn' | 'Success'}
 *  @description Status of the descriptor itself (construction/registration outcome).
 *  @default     'Success'
 *  @author      Riccardo Angeli
 *  @copyright   Riccardo Angeli 2012-2026 All Rights Reserved
 *  @license     MIT / Commercial (dual license)
 */
State : 'Fail' | 'Warn' | 'Success';
```

Tags align in a column; `@type` mirrors the TypeScript declaration (the signature is the source of truth). `//` working notes are never used as documentation — they are deleted on cleanup.
